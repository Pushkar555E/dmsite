...
const browserFrame = (image, alt, width = 1440, height = 1100) => `<div class="browser-frame"><div class="browser-bar" aria-hidden="true"><span></span><span></span><span></span><small>growth-fawn-two.vercel.app</small></div><img src="${image}" width="${width}" height="${height}" loading="lazy" decoding="async" alt="${alt}"></div>`;
const projectCards = projects.map((project) => {
  const liveAction = project.liveUrl ? `<a class="btn btn-primary" data-track="project_click" href="${project.liveUrl}" target="_blank" rel="noopener noreferrer" aria-label="Visit ${project.name} live website, opens in a new tab">Visit Live Website</a>` : '';
  const caseAction = project.caseStudyUrl ? `<a class="btn btn-secondary" href="${project.caseStudyUrl}">View Case Study</a>` : `<button class="btn btn-secondary" type="button" data-project-open="${project.id}">View Project Details</button>`;
  const coverClass = project.id === 'growth-nest' ? 'project-cover website-cover' : 'project-cover logo-cover';
  const imageAlt = project.id === 'growth-nest' ? `${project.name} live website screenshot` : `${project.name} project visual`;
  return `<article class="project-card ${project.id === 'growth-nest' ? 'featured-project-card' : ''}" data-category="${project.category.toLowerCase().split(' ')[0]}"><div class="${coverClass}"><img src="${project.image}" width="1440" height="1100" loading="lazy" decoding="async" alt="${imageAlt}"></div>${statusBadge(project.status)}<span>${project.category}</span><h3>${project.name}</h3><p>${project.background}</p><div class="project-actions">${liveAction}${caseAction}</div></article>`;
}).join('');
  ...
  <div class="modal-card" tabindex="-1">
    ...
    <div class="modal-grid">
      ...
      <div>
        ...
        <h3>Current status</h3><p>${project.status}.${project.liveUrl ? ` <a href="${project.liveUrl}" target="_blank" rel="noopener noreferrer" aria-label="Visit ${project.name} live website, opens in a new tab">Visit the live website</a>.` : ''}</p>
        ${project.caseStudyUrl ? `<a class="btn btn-secondary" href="${project.caseStudyUrl}">View Case Study</a>` : ''}
      </div>
    </div>
  </div>
</section>`).join('');
...
const home = layout({
  ...
  <section class="section-pad alt" id="portfolio"><div class="container section-head"><span class="eyebrow">Featured projects</span><h2>Real work with clear labels and direct links.</h2><p>Growth Nest is live client work. Internal and concept projects are labelled separately.</p></div><div class="container portfolio-grid">${projectCards}</div></section>
...
const growthNestCaseStudy = layout({ path: '/case-study-growth-nest.html', active: 'Case Studies', title: 'Growth Nest Website Case Study | Nexora Digital', description: 'See how Nexora Digital designed and developed the live Growth Nest digital marketing agency website, including responsive structure, service presentation and contact-focused design.', image: '/clients/growth-nest-homepage.webp', extra: `<script type="application/ld+json">${growthNestSchema}</script>`, body: `<section class="case-study-hero section-pad"><div class="container case-study-hero-grid"><div><span class="eyebrow">Client Case Study</span><h1>Growth Nest digital marketing agency website</h1><p>A live client website created to communicate services clearly, present the Growth Nest brand professionally and provide straightforward contact paths.</p><div class="hero-actions"><a class="btn btn-primary" href="${site.growthNestUrl}" target="_blank" rel="noopener noreferrer" aria-label="Visit Growth Nest live website, opens in a new tab">Visit Live Website</a><a class="btn btn-secondary" href="portfolio.html">Back to Portfolio</a></div></div>${browserFrame('/clients/growth-nest-homepage.webp', 'Growth Nest live website homepage screenshot', 1440, 1100)}</div></section>
  <section class="section-pad"><div class="container case-study-meta"><article><strong>Client</strong><span>Growth Nest</span></article><article><strong>Project type</strong><span>Digital marketing agency website</span></article><article><strong>Status</strong><span>Live and publicly accessible</span></article><article><strong>Service</strong><span>Website design and development</span></article><article><strong>Deployment</strong><span>Vercel</span></article><article><strong>Technology observed</strong><span>Next.js site output</span></article><article><strong>Role</strong><span>Website design and development</span></article></div></section>
  <section class="section-pad alt"><div class="container split-layout"><div><span class="eyebrow">Challenge</span><h2>A clearer online presence for a digital marketing agency.</h2><p>Growth Nest needed a clear online presence that could explain its services, present a professional digital identity and make it easy for potential clients to get in touch.</p></div><div class="cta-card"><h3>Objectives</h3><ul><li>Present digital marketing services clearly</li><li>Create a professional visual identity</li><li>Build a responsive experience</li><li>Create visible contact actions</li><li>Organize content for easy scanning</li><li>Prepare the site for online deployment</li></ul></div></div></section>
  <section class="section-pad"><div class="container section-head"><span class="eyebrow">Work completed</span><h2>Website structure, presentation and contact flow.</h2></div><div class="container expectation-grid"><article><h3>Website structure</h3><p>Navigation and page sections organize services, industries, pricing, blog, about and FAQ content.</p></article><article><h3>Hero and service sections</h3><p>The homepage leads with a clear growth-focused headline and service-oriented positioning.</p></article><article><h3>Calls to action</h3><p>The site uses a visible free-audit action and enquiry-focused pathways.</p></article><article><h3>Brand presentation</h3><p>Logo, typography, soft grid background and cyan-blue accents create a consistent agency identity.</p></article><article><h3>Mobile layout</h3><p>A mobile-size version is included to show how the project appears on a narrow screen.</p></article><article><h3>Deployment</h3><p>The project is live on Vercel at <a href="${site.growthNestUrl}" target="_blank" rel="noopener noreferrer" aria-label="Visit Growth Nest live website, opens in a new tab">growth-fawn-two.vercel.app</a>.</p></article></div></section>
  <section class="section-pad alt"><div class="container split-layout"><div><span class="eyebrow">Design decisions</span><h2>Simple hierarchy and direct actions.</h2><p>The design uses a spacious hero, service-led navigation, a strong free-audit CTA, and clean typography to make the agency offer easy to scan. The browser-style previews below show real captures from the live website.</p></div><div class="cta-card"><h3>Outcome</h3><ul><li>Live website successfully deployed</li><li>Responsive presentation created</li><li>Services organized clearly</li><li>Contact routes made visible</li><li>Brand given a consistent online presentation</li></ul></div></div></section>
  <section class="section-pad"><div class="container section-head"><span class="eyebrow">Screenshots</span><h2>Real captures from the live Growth Nest website.</h2></div><div class="container screenshot-grid"><figure>${browserFrame('/clients/growth-nest-homepage.webp', 'Growth Nest homepage screenshot showing hero and navigation', 1440, 1100)}<figcaption>Homepage hero and navigation.</figcaption></figure><figure>${browserFrame('/clients/growth-nest-services.webp', 'Growth Nest services section screenshot from the live website', 1440, 1100)}<figcaption>Service and page structure capture.</figcaption></figure><figure>${browserFrame('/clients/growth-nest-contact.webp', 'Growth Nest contact and call to action section screenshot from the live website', 1440, 1100)}<figcaption>Contact-oriented page capture.</figcaption></figure><figure>${browserFrame('/clients/growth-nest-mobile.webp', 'Growth Nest mobile website screenshot', 390, 900)}<figcaption>Mobile-size live capture.</figcaption></figure></div></section>
  <section class="section-pad final-cta"><div class="container cta-card"><span class="eyebrow">Current status</span><h2>Live and publicly accessible.</h2><p>Visit the live Growth Nest website or ask Nexora Digital about a website for your business.</p><div class="hero-actions"><a class="btn btn-primary" href="${site.growthNestUrl}" target="_blank" rel="noopener noreferrer" aria-label="Visit Growth Nest live website, opens in a new tab">Visit Growth Nest</a><a class="btn btn-secondary" href="contact.html?service=website-design-and-development#audit">Want a website like this?</a></div></div></section>` });

const portfolioPage = layout({ path: '/portfolio.html', active: 'Portfolio', title: 'Portfolio and Case Studies | Nexora Digital', description: 'Portfolio and case studies from Nexora Digital including the live Growth Nest client website, internal agency work, and concept frameworks.', image: '/clients/growth-nest-homepage.webp', body: `<section class="page-hero"><img src="/agency/hero-strategy-room.png" width="1200" height="750" alt="Nexora Digital project planning workspace" fetchpriority="high" decoding="async"><div class="container"><span class="eyebrow">Portfolio</span><h1>Client work, internal projects, and clear project labels.</h1><p>Growth Nest is live client work. Other projects are labelled by type so visitors know exactly what they are viewing.</p></div></section><section class="section-pad"><div class="container filter-bar" role="tablist" aria-label="Project filters"><button class="filter-btn active" data-filter="all">All</button><button class="filter-btn" data-filter="client">Client Project</button><button class="filter-btn" data-filter="internal">Internal Project</button><button class="filter-btn" data-filter="concept">Concept Project</button></div><div class="container portfolio-grid">${projectCards}</div></section><section class="section-pad alt" id="case-studies"><div class="container section-head"><span class="eyebrow">Case studies</span><h2>Detailed project notes without fake metrics.</h2></div><div class="container case-grid">${projects.map((project) => `<article class="case-card"><span>${project.category}</span>${statusBadge(project.status)}<h3>${project.name}</h3><p><strong>Overview:</strong> ${project.background}</p><p><strong>Work completed:</strong> ${project.work.join(', ')}.</p><p><strong>Visible features:</strong> ${project.features.join(', ')}.</p><p><strong>Tools or platform:</strong> ${project.tools.join(', ')}.</p><p><strong>Current status:</strong> ${project.status}.</p><div class="project-actions">${project.liveUrl ? `<a class="btn btn-primary" href="${project.liveUrl}" target="_blank" rel="noopener noreferrer" aria-label="Visit ${project.name} live website, opens in a new tab">Visit Live Website</a>` : ''}${project.caseStudyUrl ? `<a class="btn btn-secondary" href="${project.caseStudyUrl}">View Case Study</a>` : `<button class="btn btn-secondary" type="button" data-project-open="${project.id}">View Project Details</button>`}</div></article>`).join('')}</div></section>${projectModals}` });
